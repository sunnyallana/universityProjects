...
void userMenu(user* userObject, int indexUser) {
	...
	while (userMenuChoice != 5) {
		try {
			...
			cout << "           4) Know About Your Personality" << endl;
			cout << "           5) Logout Of Your Account" << endl;
			cout << "                                            " << endl;
			cout << "           Enter Choice: ";
			...
			if (userMenuChoice < 1 || userMenuChoice > 5) {
				throw out_of_range("Invalid choice. Please enter a number between 1 and 5.");
			}
			...
			switch (userMenuChoice) {
				...
				case 4: {
					userObject[indexUser].displayPersonalityDetails();
					clearScreen();
					break;
				}
				case 5: {
				...
				default: {
					throw runtime_error("Invalid choice. Please enter a number between 1 and 5.");
					break;
				}
			}
		}
