...
// Function that initializes the attributes of personality after reading from the file
void readPersonalityFromFile(personalityQuiz* quiz) {
	ifstream inFile("personalityAttributes.txt");
	if (!inFile) {
		cout << "Error opening file\n";
		return;
	}
	char mind, energy, nature, tactics, identity;
	inFile >> mind >> energy >> nature >> tactics >> identity;
	quiz->setMind(mind);
	quiz->setEnergy(energy);
	quiz->setNature(nature);
	quiz->setTactics(tactics);
	quiz->setIdentity(identity);
	inFile.close();
}
...
void populateArray(user* User) {
	...
	for (int i = 0; i < userCount; i++) {
		...
		readPersonalityFromFile(User[i].personality);
	}
	infile.close();
}
...
// Function that writes core attributes of a user to the file
