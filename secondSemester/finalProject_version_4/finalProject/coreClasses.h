...
public:
	...
	void writePersonalityToFile() const {
		ofstream outFile("personalityAttributes.txt");
		if (!outFile) {
			cout << "Error opening file\n";
			return;
		}
		outFile << this->mind << endl << this->energy << endl << this->nature << endl << this->tactics << endl << this->identity << endl;
		outFile.close();
	}
	...
	void setPersonality() {
		...
		writePersonalityToFile();
	}

	void setMind(const char receiveMind) { mind = receiveMind; }
	void setEnergy(const char receiveEnergy) { energy = receiveEnergy; }
	void setNature(const char receiveNature) { nature = receiveNature; }
	void setTactics(const char receiveTactics) { tactics = receiveTactics; }
	void setIdentity(const char receiveIdentity) { identity = receiveIdentity; }
...
private:
	bool status;
	string zodiacSign;
	personalityQuiz* personality = nullptr;
public:
	friend class admin;
	friend int main();
...
bool user::createUser(string receiveUsername, string receivePassword, string receiveName, int receiveDayOfBirth, int receiveMonthOfBirth, int receiveYearOfBirth, bool receiveStatus) {
	...
	if (fout.is_open()) {
		...
		fout.close();
		personality->writePersonalityToFile();
		return true;
	}
	return false;
}
